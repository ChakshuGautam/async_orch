...
# --- Sequence Definition ---------------------------------------------------
class Sequence:
    def __init__(self, *steps: Union[TaskFn, "Sequence", "Parallel"], name: str = None): # Changed ParallelTaskRunner to Parallel
        processed_steps = []
        for step_item in steps:
            if callable(step_item) and not isinstance(step_item, (Sequence, Parallel)): # Changed
                processed_steps.append(step_item)
            elif isinstance(step_item, (Sequence, Parallel)): # Changed
                processed_steps.append(step_item)
            else:
                raise TypeError(f"Invalid step type: {type(step_item)}. Expected TaskFn, Sequence, or Parallel.")
...
class _SequentialTaskRunner:
    def __init__(self, sequence_def: Sequence, name: str = None):
        ...
        for step_def_item in self.definition.steps:
            if callable(step_def_item) and not isinstance(step_def_item, (Sequence, Parallel)): # Changed
                self._executable_steps.append(_TaskRunner(fn=step_def_item))
            elif isinstance(step_def_item, Sequence): 
                self._executable_steps.append(_SequentialTaskRunner(sequence_def=step_def_item))
            elif isinstance(step_def_item, Parallel): # Changed
                self._executable_steps.append(_ParallelTaskRunner(parallel_def=step_def_item)) # Now uses _ParallelTaskRunner
            else:
                ...
                if isinstance(step_def_item, _TaskRunner):
                    ...
                    raise TypeError(f"Invalid step type for execution: {type(step_def_item)}")
...
# --- Parallel Definition ---------------------------------------------------
class Parallel:
    def __init__(self, *tasks: Union[TaskFn, Sequence, "Parallel"], max_workers: int = None, name: str = None):
        processed_tasks = []
        for task_item in tasks:
            if callable(task_item) and not isinstance(task_item, (Sequence, Parallel)):
                ...
                raise TypeError(f"Invalid task type: {type(task_item)}. Expected TaskFn, Sequence, or Parallel.")
...
class _ParallelTaskRunner:
    def __init__(self, parallel_def: Parallel, name: str = None):
        ...
        for task_def_item in self.definition.tasks:
            if callable(task_def_item) and not isinstance(task_def_item, (Sequence, Parallel)): # TaskFn
                self._executable_tasks.append(_TaskRunner(fn=task_def_item))
            elif isinstance(task_def_item, Sequence):
                self._executable_tasks.append(_SequentialTaskRunner(sequence_def=task_def_item))
            elif isinstance(task_def_item, Parallel):
                self._executable_tasks.append(_ParallelTaskRunner(parallel_def=task_def_item)) # Recursive
            elif isinstance(task_def_item, _TaskRunner): # Already an executable
                 self._executable_tasks.append(task_def_item)
            else:
                raise TypeError(f"Invalid task type for execution: {type(task_def_item)}")
    ...
    async def run(self) -> list[Any]:
        ...
        try:
            ...
            if (
                ...
                exception_group = exc
                if hasattr(exception_group, "exceptions") and len(exception_group.exceptions) == 1:
                    raise exception_group.exceptions[0]
            raise


# --- Circuit Breaker Definition --------------------------------------------
from aiobreaker import CircuitBreaker, CircuitBreakerError # Keep import here for CircuitBreakerError

class CircuitDefinition:
    def __init__(self, *tasks: Union[TaskFn, Sequence, Parallel], 
                 fail_max: int = 5, reset_timeout: int = 60, name: str = None):
...
class _CircuitGroupTaskRunner:
    class _EventListener:
        ...
        def state_change(self, breaker, old_state, new_state): # Made synchronous
            ...
            current_state_str = breaker.current_state # This should be the string name of new_state
    ...
    def __init__(self, circuit_def: CircuitDefinition, name: str = None):
        ...
        self.breaker = CircuitBreaker(
            ...
            timeout_duration=timedelta(seconds=self.definition.reset_timeout) if not isinstance(self.definition.reset_timeout, timedelta) else self.definition.reset_timeout
        )
        ...
        self._parallel_executor = _ParallelTaskRunner(parallel_def=self.definition.task_definition)
...
async def run(task: Union[_TaskRunner, Sequence, Parallel, CircuitDefinition]) -> Any: # Added CircuitDefinition
