...
async def test_flaky_task_succeeds_first_try(capsys):
    ...
    retry_events = [
        ...
        if e.get("task") == task_flaky and e.get("state") == TaskState.RETRYING
    ]
    assert not retry_events

    captured = capsys.readouterr()  # Check console output from log_event_for_test
    assert (
        "RETRYING" not in captured.out
    )  # Assuming log_event_for_test prints RETRYING for such states
...
async def test_flaky_task_fails_after_all_retries(capsys):
    ...
    max_retries = 3
    task_flaky = create_flaky_task_with_retry(
        name="FlakyUltimateFailure", max_tries=max_retries
    )
